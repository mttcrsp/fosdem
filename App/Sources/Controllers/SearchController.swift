...
final class SearchController: UISplitViewController {
  typealias Dependencies = HasFavoritesService & HasNavigationService & HasPersistenceService & HasTimeFormattingService & HasTracksService & HasYearsService
...
extension SearchController: TracksViewControllerDataSource, TracksViewControllerDelegate {
  ...
  func tracksViewController(_: TracksViewController, titleForSectionAt section: Int) -> String? {
    ...
    case let .day(date):
      return dependencies.timeFormattingService.weekdayWithShortDate(from: date)
    }
  }
...
private extension SearchController {
  private func reloadFilterButton() {
    ...
    if let tracksConfiguration {
      ...
      case let .day(date):
        L10n.Search.Filter.Menu.Action.day(dependencies.timeFormattingService.weekday(from: date))
      }
