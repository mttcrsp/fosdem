...
class MoreInteractor {
  typealias Dependencies = HasAcknowledgementsService & HasYearsService & HasTimeService & HasOpenService
  ...
  func didSelectCode() {
    if let url = URL.fosdemGithub {
      dependencies.openService.open(url) { _ in
        //
      }
    }
  }

  func didSelect(_ info: Info) {
    _ = info
    //
  }

  func didSelect(_ acknowledgement: Acknowledgement) {
    dependencies.openService.open(acknowledgement.url) { _ in
      //
    }
  }
}
